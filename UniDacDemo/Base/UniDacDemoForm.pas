...
{$IFDEF PostgreSQL} PostgreSQLUniProvider, {$ENDIF}
{$IFDEF SQLite} SQLiteUniProvider, {$ENDIF}
{$IFNDEF LINUX}
  {$IFDEF ODBC} ODBCUniProvider,{$ENDIF}
  {$IFDEF DB2} DB2UniProvider,{$ENDIF}
  {$IFDEF Access} AccessUniProvider,{$ENDIF}
  {$IFDEF Advantage} AdvantageUniProvider,{$ENDIF}
  {$IFDEF ASE} ASEUniProvider, {$ENDIF}
  System.ImageList
{$ENDIF}
{$ELSE}
...
type
  ...
  public
    /// <summary>
    /// 根据数据库连接数据提供者名称的不同，选择不同的创建数据库结构的脚本
    /// </summary>
    procedure ExecCreateScript; override;

    /// <summary>
    /// 根据数据库连接数据提供者名称的不同，选择不同的删除数据库结构的脚本
    /// </summary>
...
const
  ...
  SODBC = 'ODBC';
  SSQLite = 'SQLite';
  SPostgreSQL = 'PostgreSQL';
  SMySQL = 'MySQL';
  SSQLServer = 'SQL Server';
...
begin
  if UniConnection.ProviderName = SOracle then
    scCreate_Oracle.Execute
  else
  if UniConnection.ProviderName = SInterBase then
    scCreate_InterBase.Execute
  else
  if UniConnection.ProviderName = SSQLServer then
    scCreate_SQLServer.Execute
  else
  if UniConnection.ProviderName = SMySQL then
    scCreate_MySQL.Execute
  else
  if UniConnection.ProviderName = SPostgreSQL then
    scCreate_PgSQL.Execute
  else
  if    (UniConnection.ProviderName = SSQLite) //
     or (UniConnection.ProviderName = SODBC)
  then
    scCreate_SQLite.Execute
  else
  if UniConnection.ProviderName = SDB2 then
    scCreate_DB2.Execute
  else
  if UniConnection.ProviderName = SAccess then
    scCreate_Access.Execute
  else
  if UniConnection.ProviderName = SAdvantage then
    scCreate_Adv.Execute
  else
  if UniConnection.ProviderName = SASE then
...
begin
  ...
  if   (UniConnection.ProviderName = SSQLite) //
    or (UniConnection.ProviderName = SODBC) //
    or (UniConnection.ProviderName = SAccess) //
  then
